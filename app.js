...
let db = null;
const initializationOfDbAndServer = async (req, res) => {
  try {
    db = await open({
      filename: dbPath,
    ...
    app.listen(3000, () => {
      console.log(`DB is running at http://localhost:3000`);
    });
  } catch (e) {
...
const tweetAccessVerification = async (request, response, next) => {
  ...
  const getTweetQuery = `select 
    * from tweet INNER JOIN follower ON tweet.user_id=follower.following_user_id where 
...
app.get(
  ...
  async (request, response) => {
    ...
    from user inner join reply on user.user_id =reply.user_id where 
    tweet_id="${tweetId}"`;
